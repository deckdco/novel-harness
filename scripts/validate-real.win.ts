/**
 * 真实数据端到端验证（Windows 本机版）：直接调用 7 个工具的 execute 逻辑（不经模型）。
 * 全部只读操作，不向真实项目写入任何文件。
 * 与 validate-real.ts 的区别仅在 root 路径适配 Windows。
 * 运行：node --experimental-strip-types --no-warnings scripts/validate-real.win.ts
 */
...
const config: ProjectConfig = {
  root: 'D:/path/to/齐国大航海/v2',  // ← 改为你本机小说项目根目录
  files: {
    bible: '../v1/00_设定圣经.md',
    master: '齐国大航海-v3架构总纲（海洋结局·爽文版）.md',
    outline: '齐国大航海-v3千章卷纲.md',
    methodology: '存道-网文写作方法论.md',
    bridges: '存道-亮点桥段设计.md',
    preference: '存道-作者偏好档案.md',
  },
  chaptersDir: '正文',
  finalVariant: '定稿',
...
const feedback = feedbackTool(project)
...
await run('feedback digest（尚无定稿溯源，应为空+建档提示）', () => feedback.execute({ action: 'digest' }, {} as never))
